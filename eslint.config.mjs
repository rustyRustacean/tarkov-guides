...
const eslintConfig = defineConfig([
  ...
  ...tseslintConfigs.stylisticTypeChecked,
  {
    languageOptions: {
      parserOptions: {
        ...
        // the app build) and the plain-JS web workers served from public/
        // (companion-tick-worker.js - shipped as a static file, never
        // bundled) aren't part of tsconfig.json's `include` and don't
        // need type-aware linting - fall back to a default (untyped)
        // project for them instead of erroring.
        projectService: {
          allowDefaultProject: ["*.config.mjs", "scripts/*.mjs", "public/*.js"],
        },
  ...
  {
    files: ["*.config.mjs", "scripts/*.mjs", "public/*.js"],
