 ...
 * `PersistenceAdapter` interfaces (pre-production audit, `CODE_AUDIT.md`
 * finding 8) - each feature still declares its own narrower type extending
 * this with a concrete `id` union (e.g. `"local-storage" | "fsa-folder" |
 * "manual-json"`), since the two features support different backend sets.
 *
 ...
 * reference to e.g. `adapter.write` (as this project's adapter tests do, via
 * `vi.mocked(...)`/`expect(...)`).
 */
