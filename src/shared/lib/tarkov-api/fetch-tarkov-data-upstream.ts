 ...
 *
 * **Server-only**: called exclusively by `src/app/api/tarkov-data/route.ts`,
 * never directly from client code. See that route's doc comment for why
 * (the combined payload is too large for Next's per-fetch Data Cache, so
 * the route's own `revalidate` config is what actually caches it, shared
 ...
 * directly) so the two are never confused at a glance, and so every
 * existing test mocking the client-facing `fetchTarkovGameData` name keeps
 * working unmodified.
 *
...
export async function fetchTarkovDataUpstream(
  ...
  const settled = await Promise.allSettled([
    ...
    // real PvE-tagged fetch: most tasks are shared between PvP and PvE, but
    // some have PvE-specific requirements or are PvE-exclusive, so this is
    // worth pulling distinctly, mirroring the PvE-price `items` fetch above.
    // Seasonal PvP has no equivalent: tarkov.dev's `GameMode` enum is
    // confirmed (against its live schema) to only have `regular`/`pve`, no
    // `season` value, so Seasonal transparently reuses the "regular" tasks
    // below (see `useActiveModeTasks`'s `isAccurateForMode`).
