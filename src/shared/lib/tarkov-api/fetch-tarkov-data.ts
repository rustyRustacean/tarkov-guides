...
/**
 * The client's sole entry point into tarkov.dev game data (2026-07-16
 * caching audit) - fetches the already-unwrapped
 * `RawTarkovApiResponseData` from this app's own server-side proxy rather
 * than tarkov.dev directly. Before this, every visitor's browser called
 * tarkov.dev independently (React Query's `staleTime` only bounds how often
 * a SINGLE browser refetches, not how many browsers exist), so upstream load
 * scaled with visitor count. The proxy route's own cached response (shared
 * across every request, not per-browser - see that route's doc comment for
 * exactly which Next.js mechanism achieves this) now caps that at roughly
 * one upstream call per revalidation window (1 hour), total.
 *
