 ...
 * independently re-deriving its own `itemsById`/`tasksById` on mount.
 * Pre-production audit (`CODE_AUDIT.md` finding 7) found 8 components each
 * hand-rolling this exact `new Map(items.map(...))`/`new Map(tasks.map(...))`
 * pattern - real, repeated allocation over the live catalog (~5,000 items)
 * and task list (~510 tasks) that this hook replaces.
 *
