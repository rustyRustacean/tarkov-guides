 ...
 * implementation - see `old/TarkovTrackerWB-main/src/lib/flea.js`).
 *
 * Deliberately does NOT apply the Intel Center 3 (-30%) or Hideout
 * Management skill (-0.3%/level) discounts - those are personal, per-player
 * modifiers that would mislead anyone without them. This returns the raw
 * tax a fresh account would pay.
 *
...
export function calculateFleaTax(basePrice: number, listPrice: number, count = 1): number {
  ...
  // Conditional exponentiation only ever applies to the POSITIVE log value -
  // whichever side is above zero gets amplified. The other side stays
  // negative and 4^negative just makes a fraction. This avoids the
  // Math.pow(negative, 1.08) = NaN issue a naive port would hit.
 ...
 * `count` produced an internally-inconsistent result (tax computed as if
 * count=1, gross computed as a negative number clamped to 0). This port
 * applies the same `Math.max(1, ...)` floor in both places.
 *
