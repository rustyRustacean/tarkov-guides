...
interface ItemDetailStoreState {
  ...
  back: () => void;
  /** Closes the whole drilldown; next open starts fresh, matching legacy's `closeTask()`. */
  close: () => void;
}
 ...
 * store). Any item row anywhere can call
 * `useItemDetailStore.getState().openItem(id)` from a plain event handler,
 * with no per-call-site dialog state to thread.
 *
 * `current` + `stack` together model the drilldown chain: opening a popup
 * pushes whatever was showing onto `stack`; {@link back} pops it. This is
 * separate from Progress Tracker's own locally-mounted `QuestDetailDialog`
 * instances (which keep their own `selectedTaskId` state); those are
 * unaffected, since this store only powers cross-surface item clicks.
 */
...
/** Opens an item-detail popup from anywhere, not just components (e.g. plain event handlers). */
