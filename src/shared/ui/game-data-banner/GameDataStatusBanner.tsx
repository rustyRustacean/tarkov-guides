 ...
 * so every route benefits, not just components that read task/item/map
 * data directly. Silent by default: `useTarkovGameData` already keeps
 * serving the last successful fetch through a transient outage
 * (`use-tarkov-game-data.ts`'s merge-with-previous safety net), so most
 * visitors never see this at all. It only appears once a fetch has
