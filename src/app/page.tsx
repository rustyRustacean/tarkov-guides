...
// Disabled per user request; commented out below along with the cards that
// used it and the sample placeholder images, rather than deleted, so it can
// be restored later without re-deriving the shape/copy.
 ...
 * has already been swapped for a real in-app screenshot of the Terminal map
...
// destination routes yet, so each renders as an inert, disabled action.
//
// Disabled per user request; see `ComingSoonFeature`'s doc comment above
// for why it's commented out rather than removed.
    ...
    <div>
      <section className="relative overflow-hidden">
        {/* Positioned first (behind, via normal DOM/paint order) rather
            ...
            order plus a positive z-index on the content above it is
            stacking-context-proof regardless of any theme's page-level CSS.
        ...
        {/* TEMP experiment, per user request: card below disabled to
            preview a full-bleed, unmasked blur across the whole river
            instead. `backdrop-blur-[3px]` is the exact value the old
            gradient-masked band used: its mask went fully opaque (`black`)
            at the center behind the title, so 3px was always the strongest
            blur it ever rendered; nothing stronger to pull from.
        ...
        {/* Disabled for the full-bleed blur experiment above: this is the
            card being previewed against. Restore by deleting the two divs
            above (the comment + the unmasked blur layer) and uncommenting
            this block in place of the plain wrapper above it.
        <div className="relative z-10 mx-auto max-w-3xl px-4 pt-24 pb-12 sm:pt-32 sm:pb-16">
