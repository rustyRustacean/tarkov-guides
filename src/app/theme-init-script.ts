 ...
 * its only job is picking which of the 6 `data-theme` values applies and
 * setting it synchronously, so the correct theme's CSS is active from the
 * very first frame. Midnight's own light/dark split needs no JS at all (pure
 ...
 *
 * Kept as a plain string (rather than inline JSX) so its logic is
 * unit-testable in isolation without a full browser - see
