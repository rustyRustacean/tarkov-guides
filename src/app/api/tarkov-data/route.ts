...
/**
 * Server-side shared cache in front of tarkov.dev's GraphQL API (2026-07-16
 * caching audit). Before this route existed, every
 ...
 * exist, so total upstream load scaled with visitor count rather than
 * staying a fixed constant.
 *
 * **What actually caches this route, confirmed empirically, not assumed
 * from docs:** the obvious-looking approach - annotating
 * `fetchTarkovDataUpstream`'s own outbound `fetch` with `next: { revalidate
 * }` - does NOT work here. A real production build logged `Failed to set
 * Next.js data cache for https://api.tarkov.dev/graphql, items over 2MB can
 * not be cached` - the combined response (~7-10MB, dominated by the `items`
 * catalog) exceeds Next's fetch-level Data Cache's 2MB-per-entry limit, so
 * that annotation would silently do nothing. What actually works is this
 * file's own `export const revalidate` above (Route Segment Config), which
 * caches the ROUTE'S OWN RESPONSE as a static/ISR artifact - a different
 * Next.js mechanism than the per-fetch Data Cache, with no matching size
 * limit observed. Verified directly: rebuilt with and without the
 * (non-functional) `next.revalidate` fetch annotation and confirmed
