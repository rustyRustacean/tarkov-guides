...
/**
 * Stable module-level array for `extraWriters` (see
 * `useStorePersistenceSync`'s doc comment for why this can't be a fresh
 * literal at the call site). `fsaFolderAdapter.write` no-ops internally when
 * no folder is linked, so this entry is safe even when Tier 2 isn't in use.
 */
const EXTRA_WRITERS = [fsaFolderAdapter];

/**
 * Keeps localStorage (Tier 1), and the FSA folder (Tier 2) when linked, in
 * sync with the store, plus the cross-tab `storage`-event counterpart that
 * keeps two tabs on the same profile from overwriting each other. Built on
 * `useStorePersistenceSync` (see that hook's own doc comment for the
 * debounce/flush/cross-tab mechanics); this feature's own contribution is
 * its snapshot type, its two adapters, `STORAGE_KEY`, and `isSuspended`
