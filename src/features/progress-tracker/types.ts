...
export interface TaskProgress {
   ...
   * without resorting to tarkov-tips's fake linear-interpolation one.
   * Optional and absent for any pre-existing/imported data recorded before
   * this field existed.
   */
  completedAt?: string;
}
...
/**
 * A game mode a profile can track progress in. `PVP_SEASONAL` exists
 ...
 * map-annotation/map-display fields, which belong to the Maps feature.
 *
 * `faction` lives here rather than on `Profile`: each mode is effectively
 * its own in-game character, and can have its own faction (e.g. BEAR in
 * PvP, USEC in PvE for the same profile). Immutable once a bucket exists,
 * same convention as the old `Profile.faction`: no setter is ever exposed
 * for it.
 */
export interface ProfileProgress {
  ...
  /** Per-profile: fixes a legacy bug where the goal was global despite depending entirely on per-profile `hideoutBuilt` state. */
   ...
   * `getKappaItems`, matching legacy scoping: not every
   ...
   * confirmed via wiki cross-reference that tarkov.dev's field means "must
   * already have this Prestige level."
   */
 ...
 * what used to be "the" mode and faction of a profile are now per-mode-bucket
 * fields on `ProfileProgress`, since one profile can hold up to 3
 * independent mode-characters at once.
 */
