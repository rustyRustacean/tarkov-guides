 ...
 * variant id for the same physical location (confirmed live: every current
 * Ground Zero task lists `"ground-zero"` and `"ground-zero-21"` together),
 * so without this canonicalization step the variant would surface as its
 * own duplicate location card carrying the exact same tasks.
 */
