 ...
 * per-map variant selections from `sessionStorage` in a separate mount
 * effect, kept out of the durable snapshot and out of the store's initial
 * state so SSR and the client's first render match. This is Maps' own
 * extra concern since it's the only feature that needs it.
 */
