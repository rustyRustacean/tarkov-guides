 ...
 * stroke authored by `authorId`. This is the reintroduction of the legacy
 * multi-contributor undo behavior `annotations.ts`'s `undoStroke` doc
 ...
 */
export function findOwnStrokeToUndo(strokes: readonly Stroke[], authorId: string): Stroke | null {
 ...
 * removed strokes, keyed on each stroke's own stable `id`. Used to translate
 * `AnnotationCanvas`'s "here's the whole new layer" callback into targeted
 * per-stroke Liveblocks `LiveMap` set/delete calls
 ...
 * either adds a whole new stroke or removes one outright (see
 * `annotations.ts`'s `eraseNear`/`replaceStroke`, which always mint fresh ids
 * for any surviving/edited stroke rather than mutating one in place).
 */
