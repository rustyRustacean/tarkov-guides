 ...
 * live `src/time.ts` (2026-07-19 accuracy audit) - same 7x ratio, same +3h
 * Moscow anchor, same +12h right-side split. Not a porting bug.
 *
 ...
 * against (confirmed via schema search, 2026-07-19), and BSG has changed
 * the real accleration ratio across game history (4:1 in 2017, ~7.5:1 in
 * 2019, settling on 7:1 later per the official forum). A pure epoch-anchored
 * formula has no way to self-correct if the true ratio ever isn't *exactly*
 * 7 - any tiny deviation compounds over the ~56 years since epoch into a
 * visible, permanent offset. `TARKOV_CLOCK_CALIBRATION_OFFSET_MS` below
 * exists to absorb exactly that drift.
 *
 ...
 * measured 2026-07-19: displayed clock read 05:26/17:26 while the actual
 * in-raid clock read ~05:37/17:37, an 11-minute gap. See the module doc's
 * "Re-calibrating this constant" section before changing this value.
 */
