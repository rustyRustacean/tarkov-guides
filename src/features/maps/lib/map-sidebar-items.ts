 ...
 * {@link getTrackedItems} selector rather than a parallel aggregator (a
 * deliberate decision - see the Phase 5 step 9 plan). Tasks are pre-filtered
 * to `inprog` + relevant to this map before being handed to the selector, so
 * item need/have/remaining math stays identical to Progress Tracker's own
 * Items tab. `getTrackedItems` always includes custom items regardless of
 * which tasks are passed in (matching legacy's "custom items show on every
 * map"), but also falls back to including every pinned item app-wide even
 * when unrelated to this map - those fallback rows (`source === "pinned"`)
 * are filtered back out here so pinning an item on one map doesn't make it
 * show on every other map too.
 */
