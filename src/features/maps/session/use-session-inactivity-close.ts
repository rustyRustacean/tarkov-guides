 ...
 * isn't precisely timed or under this app's control; the explicit
 * `/api/maps-session/end` route is the primary expiry mechanism.
 * "Activity" is any storage change at all: selecting the whole root (not
 * one field) so a map/variant switch, a pan/zoom broadcast, or a stroke all
 * reset the clock, not just whichever field this hook happens to name.
 *
...
export function useSessionInactivityClose(
  ...
  // first run of the effect below (which fires on mount, since `active`/
  // `storageSnapshot` are already set by then) is what records the real
  // starting timestamp.
